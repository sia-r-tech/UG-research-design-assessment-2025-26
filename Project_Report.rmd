...
author: 'Exam Numbers: B247222, B218357, B249236'
date: "`r Sys.Date()`"
output:
  html_document:
    code_folding: hide
...
```{r load-wrangle}
# load packages needed for all figures
library(tidyverse)
library(janitor)
library(here)
library(ggrepel)
...
improved_plot <- ggplot(fig3_faceted, aes(x = category_short, y = percentage, fill = group_short)) +
  ...
  # BARS
  geom_col(position = position_dodge(width = 0.9), width = 0.8, color = "black", linewidth = 0.3) +
  ...
  # TITLES
  labs(
    title = "Figure 3 Postpartum Depression Severity by Group",
...
# Keep only those with values for all three time points
...
# Recode the groups, and set the time order
...
# Calculate means and SDs at each time point
fs_sum <- fs_long %>%
  group_by(group, time) %>%
  summarise(
    n = n(),
...
fig4 <- ggplot(fs_sum, aes(x = time, y = mean, group = group, linetype = group)) +
  ...
  # Mean labels (custom x shift and vertical shift)
  geom_text(
    aes(
      label = label,
      y = mean + label_y,
...
### Improved Figure 4
```{r figure-4-improved}
# Dodge to seperate HIIT and EDU so SD bars don't overlap
...
# Build the improved plot with dodged SD, points and lines and better group coding in legend
...
The new graph improves readability by dodging the two groups slightly. By doing this, it prevents the standard deviation error bars from overlapping, making interpretation easier. We also made the group identity clearer by creating filled and open points (HIIT-filled points, EDU-open points). The mean labels are made into clearer group labels and are moved to avoid collisions. The y-axis range was made slightly narrower to highlight the differences between the lines better. The legend is simpler and has longer line indicators (solid vs dotted) that makes the distinction between the groups visibly apparent.
...
The dataset meets several aspects of the FAIR data principles. It is publicly available in a recognised repository (Mendeley Data) and has been assigned a persistent DOI, so it can be easily found and cited. The data has a description, a clear title and authors, and has been assigned to categories which add to its findability. However, there is limited metadata, particularly because there is no data dictionary that explains the variables. The dataset could therefore include variable-level descriptions on each column, with units and coding.

In terms of accessibility, the dataset is openly accessible via a free and universally used protocol (https) and has no restrictions. Since the data is anonymised, an authentication procedure is not needed. Additionally, since it has a DOI, the metadata would remain accessible even if the data were removed. However, a description of how to access the data, given in Excel (proprietary format), would improve accessibility.

Looking at interoperability, although the data is presented in a standard table format, it is given as an Excel file, which could limit its compatibility in some workflows. Additionally, variable names are unclear and not matched to standard vocabularies (e.g., for depression scores). By using a non-proprietary format (e.g., csv) and standardised terminology it would become easier to integrate into other datasets.

The reusability of the dataset is strengthened by a clear licence, a brief description and linked publications. However, there is no documentation or instructions on how the data can be used. Including a codebook that describes each attribute of the data, or code examples that show how the data was processed and analysed in the publications, would improve reusability.

Overall, while the data is open and easy to access, its FAIR-ness could be strengthened by using standardised variable names, variable-level metadata, and non-proprietary formats. 

## Task 3: Reproducibility brief

Your study has some good aspects that make it reproducible. You have made your dataset publicly available in a trusted repository and linked it to published articles that use it. This means others can find and access the data to reproduce your work. You also describe the intervention, outcomes, and study design in detail, which makes your work easier to understand and gives a good idea of your workflow.

However, your workflow still needs to be more transparent. Although the results can be understood from your descriptions, it would be difficult for someone to follow the exact steps you took in your analysis and get the same results. You could add a link to the code that you used to clean and analyse the data (for example in a GitHub repository). In this repo put any scripts you used, e.g., preprocessing, generating figures, and statistical analysis.

Second, I would recommend documenting your variables more clearly. An option would be to add a README or a codebook that answers questions like: how were variables calculated, how did you handle missing data, which participants did you exclude? After this, you could add a short file that gives instructions on how to run your code so that it is easier to follow your workflow.

I like how you specify which software environments and packages you used for the analysis and included the specific versions and parameters. However, instead of simply adding figures and brief notes as linked supplementary material, you could go into more detail on how you used each package (e.g., syntax for ITT) in a separate ‘instructions’ document and add which packages are required as an appendix.

These changes would make it easier for others to go through your pipeline, perform the same analysis steps, and get the same results.
